/* eslint-disabled max-nested-callbacks */
...
window.getRefreshRate = function () {
	return new Promise((resolve, _reject) => {
		const knownRefreshRates = [60, 75, 100, 120, 144, 165, 240];
		...
		setTimeout(() => {
			requestAnimationFrame((start) => {
				requestAnimationFrame((end) => {
					...
					// Get the closest known refresh
					const closest = knownRefreshRates.reduce((prev, curr) =>
						Math.abs(curr - rate) < Math.abs(prev - rate)
							? curr
							: prev,
					);
...
Math.oscilate = function (
	...
	const angularFrenquecy = 2 * Math.PI * cyclesPerSecond;

	// Calculate the amplitude range
	const amplitudeRange = maxAmplitude - minAmplitude;

	// Calculate the the sine wave value at the given time
	const sineValue = Math.sin(angularFrenquecy * time);

	// Scale the sine value to the amplitude range
	const scaledValue = (sineValue + 1) / 2;
...
Math.prettifyElapsedTime = function (ms) {
	const toFixed = (value, digist) => {
		if (value % 1 === 0) return Math.floor(value);
		else return value.toFixed(digist);
	};
...
CanvasRenderingContext2D.prototype.fillTextAligned = function (
	text,
	x,
	y,
	align,
