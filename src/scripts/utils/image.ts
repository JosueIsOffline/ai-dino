...
export abstract class ImageUtils {
    ...
    public static load(id: string, size: Optional<Size> = null): Promise<HTMLImageElement> {
        return new Promise((resolve, reject) => {
            if (this.cache.has(id)) {
               ...
               if(value === undefined) throw new Error("Image is undefined");
            ...
            image.onerror = (e) => {
                Log.error("ImageUtils", `Failed to load image with id: ${id}`, e.toString());
                reject(e);
            }
    ...
    public static pixelPerfectCollision(
        aPosition: Vector,
        aSprite: Sprite,
        bPosition: Vector,
        bSprite: Sprite
